...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    public void insert(T key, int[] set) {
        set[1]++;
        insert(new RedBlackNode<T>(key),set);
    }
    ...
    private void insert(RedBlackNode<T> z, int[] set) {
    ...
    public RedBlackNode<T> search(T key){
        ...
        while (!isNil(current)){
            ...
            {
                return current;// return that node and exit search(int)
            }
    ...
    // Searches for a node with key k and returns the first such node, if no
    // such node is found returns null
    public RedBlackNode<T> search(T key, int[] set){
    ...
    //return true if the given key exist in the tree otherwise will return flase
    public boolean exist(T key, int[] set)
    {
        RedBlackNode<T> current = root;
        return exist(current,key,set);
    }
    // function to search a given key in Black red tree
    private boolean exist(RedBlackNode<T> current, T key, int[] set)
    {
        ...
        {
            set[0]++;
            return exist(current.left, key, set);
        }
        else 
        {
            set[0]++;
            return exist(current.right, key, set);
        }
    }
